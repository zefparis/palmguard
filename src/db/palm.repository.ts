...
class SupabaseRepository implements PalmRepository {
  ...
  async enroll(record: EnrollRecord): Promise<void> {
    ...
    const { error } = await this.client.from("palm_enrollments").insert({
      ...
      kem_privkey_enc:     Buffer.from(record.kemPrivkeyEnc).toString("base64"),
      kek_iv:              Buffer.from(record.kekIv).toString("base64"),
      captured_at:         new Date(record.capturedAt).toISOString(),
      celestial_jdn:       record.celestialJdn,
  ...
  async findEnrollment(tenantId: string, userId: string): Promise<EnrollRecord | null> {
    ...
    return {
      ...
      kemPrivkeyEnc:      Buffer.from(data.kem_privkey_enc as string, "base64"),
      kekIv:              Buffer.from(data.kek_iv as string, "base64"),
      capturedAt:         new Date(data.captured_at as string).getTime(),
      celestialJdn:       data.celestial_jdn as number,
